...
# وبعدها (اختياري) يفحص أسماء النماذج عند بوابة DeepSeek إن وُجد المفتاح
#
# الاستخدام:
...
# ------------------------------------------------------------
# ٥. فحص أسماء النماذج عند بوابة DeepSeek (اختياري — لا يفشل التثبيت)
# ------------------------------------------------------------
# فتحة sonnet مضبوطة على اسم تجريبي قد يغيّره المزود دون إنذار.
# الفحص استشاري: بلا مفتاح أو شبكة يحذّر ويتخطّى.
CM="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/check-model-names.sh"
if [ -f "$CM" ]; then
  step "٥. فحص أسماء النماذج عند بوابة DeepSeek (اختياري)"
  if ! bash "$CM"; then
    warn "فحص أسماء النماذج لم يكتمل — راجع التحذير أعلاه"
  fi
else
  warn "check-model-names.sh غير موجود — تخطّي فحص أسماء النماذج"
fi
